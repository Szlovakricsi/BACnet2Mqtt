...
mqtt.on("connect", async () => {
  ...
  try {
    if (!gateway && !startingGateway) {
      ...
      // Always allow legitimate external Who-Is discovery requests, including
      // requests originating from the same host/IP (for example YABE running
      // on the Home Assistant host). Only honour the optional BACnet device
      // instance range when one is supplied by the requester.
      const answerWhoIs = haBacnet.handleWhoIs.bind(haBacnet);
      haBacnet.handleWhoIs = async req => {
        ...
        if (haBacnet.config?.enabled === true) {
          const sender = req?.header?.sender;
          const senderAddress = String(sender?.address || "unknown");
          const senderPort = Number(
            sender?.port ??
            sender?.udpPort ??
            options.bacnet_port
          );
          gateway.log.info(
            `HA→BACnet Who-Is from ${senderAddress}:${senderPort}; ` +
            `responding as Device ${deviceId}`
          );
        }

        return answerWhoIs(req);
      };

      // The shared UDP socket can receive the I-Am response generated by the
      // virtual Home Assistant device itself. Ignore that single Device ID in
      // the physical-controller discovery path instead of suppressing Who-Is.
      // The BACnet event listener calls gateway.handleIAm dynamically, so this
      // instance wrapper also covers later manual scans and Device ID changes.
      const handlePhysicalIAm = gateway.handleIAm.bind(gateway);
      gateway.handleIAm = async data => {
        const deviceId = Number(data?.payload?.deviceId);
        const virtualDeviceId = Number(haBacnet?.config?.deviceId);

        if (
          haBacnet?.config?.enabled === true &&
          Number.isFinite(deviceId) &&
          Number.isFinite(virtualDeviceId) &&
          deviceId === virtualDeviceId
        ) {
          gateway.log.debug(
            `Ignoring virtual Home Assistant I-Am for Device ${deviceId} ` +
            `in physical BACnet discovery`
          );
          return;
        }

        return handlePhysicalIAm(data);
      };
